      ...
      <div className="relative z-10 w-full pt-16 sm:pt-20 pb-20 sm:pb-24 px-4 sm:px-6">
        ...
        <div className="bg-primary/5 border-2 border-primary/20 rounded-xl p-4 text-center">
          ...
          <label className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold cursor-pointer hover:bg-primary/90 transition-colors shadow-ambient">
            ...
            Subir o hacer foto
            <input type="file" accept="image/png,image/jpeg,image/webp,application/pdf" capture="environment" className="hidden" onChange={(e) => {
              if (!e.target.files) return;
              const MAX_SIZE = 10 * 1024 * 1024;
              const ALLOWED = new Set(["application/pdf", "image/png", "image/jpeg", "image/webp"]);
              const picked = Array.from(e.target.files);
              const tooBig = picked.find((f) => f.size > MAX_SIZE);
              if (tooBig) { setError(t("upload.errorSize")); return; }
              const badMime = picked.find((f) => f.type && !ALLOWED.has(f.type));
              if (badMime) { setError(t("upload.errorMime")); return; }
              setError(null);
              setUploadedFiles(picked);
            }} />
          </label>
          {uploadedFiles.length > 0 && (
            <p className="text-xs text-primary font-medium mt-2">{uploadedFiles.map((f) => f.name).join(", ")}</p>
          )}
          <p className="text-[10px] text-muted-foreground mt-1">{t("upload.limits")}</p>
        </div>
